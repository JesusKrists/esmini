...
void OSIReporter::ReportSensors(std::vector<ObjectSensor *> sensor)
{
  ...
  for (unsigned int i = 0; i < sensor.size(); i++)
  {
    ...
    for (unsigned int j = 0; j < static_cast<unsigned int>(sensor[i]->nObj_); j++)
    {
      ...
      mobj->mutable_base()->mutable_position()->set_x(
          sensor[i]->hitList_[j].x_ + static_cast<double>(sensor[i]->hitList_[j].obj_->boundingbox_.center_.x_) * cos(sensor[i]->hitList_[j].yaw_));
      mobj->mutable_base()->mutable_position()->set_y(
          sensor[i]->hitList_[j].y_ + static_cast<double>(sensor[i]->hitList_[j].obj_->boundingbox_.center_.x_) * sin(sensor[i]->hitList_[j].yaw_));
...
int OSIReporter::UpdateOSIStationaryObjectODR(int road_id, roadmanager::RMObject *object)
{
  ...
  else if (object->GetType() == roadmanager::RMObject::ObjectType::BUILDING)
  {
    obj_osi_internal.sobj->mutable_classification()->set_type(
        osi3::StationaryObject_Classification_Type::StationaryObject_Classification_Type_TYPE_BUILDING);
  ...
  else if (object->GetType() == roadmanager::RMObject::ObjectType::OBSTACLE || object->GetType() == roadmanager::RMObject::ObjectType::PARKINGSPACE ||
           ...
           object->GetType() == roadmanager::RMObject::ObjectType::CROSSWALK || object->GetType() == roadmanager::RMObject::ObjectType::STREETLAMP ||
           object->GetType() == roadmanager::RMObject::ObjectType::GANTRY || object->GetType() == roadmanager::RMObject::ObjectType::SOUNDBARRIER ||
           object->GetType() == roadmanager::RMObject::ObjectType::WIND || object->GetType() == roadmanager::RMObject::ObjectType::ROADMARK)
  {
    obj_osi_internal.sobj->mutable_classification()->set_type(
               ...
               incomming_road->GetDrivingLaneById(incomming_s_value, junctionlanelink->to_) != 0 && !right_hand_traffic))
          {
            ...
            if (lane != nullptr)
            {
              laneparing->mutable_successor_lane_id()->set_value(static_cast<unsigned int>(
                  outgoing_road->GetDrivingLaneById(outgoing_s_value, lane->GetLink(connecting_road_link_type)->GetId())->GetGlobalId()));
            }
            else
            {
      ...
      for (int k = 0; k < IntersectionLanes[j]->classification().lane_pairing_size(); ++k)
      {
        ...
        if (IntersectionLanes[j]->classification().lane_pairing()[k].has_antecessor_lane_id())
        {
          ...
          if (obj_osi_internal.gt->lane(i).id().value() == IntersectionLanes[j]->classification().lane_pairing()[k].antecessor_lane_id().value())
          {
            ...
            {
              obj_osi_internal.gt->mutable_lane(i)->mutable_classification()->add_lane_pairing()->mutable_successor_lane_id()->set_value(
                  IntersectionLanes[j]->id().value());
            }
          }
        }
        ...
        if (IntersectionLanes[j]->classification().lane_pairing()[k].has_successor_lane_id())
        {
          ...
          if (obj_osi_internal.gt->lane(i).id().value() == IntersectionLanes[j]->classification().lane_pairing()[k].successor_lane_id().value())
          {
            ...
            if (obj_osi_internal.gt->mutable_lane(i)->mutable_classification()->lane_pairing_size() == 0)
            {
              obj_osi_internal.gt->mutable_lane(i)->mutable_classification()->add_lane_pairing()->mutable_antecessor_lane_id()->set_value(
                  IntersectionLanes[j]->id().value());
            }
          }
        }
      ...
      for (int k = 0; k < lane_section->GetNumberOfLanes(); k++)
      {
        ...
        if ((!lane->IsCenter() && !lane->IsOSIIntersection()))
        {
          ...
          if (!osi_lane)
          {
            ...
            else if (lanetype == roadmanager::Lane::LaneType::LANE_TYPE_SPECIAL1 || lanetype == roadmanager::Lane::LaneType::LANE_TYPE_SPECIAL2 ||
            ...
            else
            {
              ...
              int next_lane_id = 0;
              if (lane_id <
                  0)  // if lane is on the right, then it contains its right boundary. So I need to look into its left lane for the left boundary
              {
                next_lane_id = lane_id + 1;
              }
              else if (lane_id >
                       0)  // if lane is on the left, then it contains its left boundary. So I need to look into its right lane for the right boundary
              {
                next_lane_id = lane_id - 1;
              }
