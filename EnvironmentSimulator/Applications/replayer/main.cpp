...
void ReportKeyEvent(viewer::KeyEvent* keyEvent, void* data)
{
  ...
  if (keyEvent->down_)
  {
    ...
    {
      if (keyEvent->modKeyMask_ & static_cast<int>(ModKeyMask::MODKEY_CTRL) && keyEvent->modKeyMask_ & static_cast<int>(ModKeyMask::MODKEY_SHIFT))
      {
...
int main(int argc, char** argv)
{
  ...
  try
  {
    ...
    if ((arg_str = opt.GetOptionArg("info_text")) != "")
    {
      ...
      viewer->SetNodeMaskBits(viewer::NodeMask::NODE_MASK_INFO | viewer::NodeMask::NODE_MASK_INFO_PER_OBJ, mask * viewer::NodeMask::NODE_MASK_INFO);
    }
                  ...
                  state && state->info.visibilityMask != 0)  // and objects invisible for graphics, traffic and sensors
              {
                if (separating_axis_intersect(scenarioEntity[0], scenarioEntity[i]))
                {
                  ...
                  if (!overlap)
                  {
                    ...
                    pause_player = true;
                    double rel_speed =
                        abs((player->GetState(scenarioEntity[0].id))->info.speed - (player->GetState(scenarioEntity[i].id)->info.speed)) * 3.6f;
