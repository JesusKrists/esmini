...
void ScenarioEngine::prepareGroundTruth(double dt)
{
  for (size_t i = 0; i < entities_.object_.size(); i++)
  {
    ...
    if (frame_nr_ == 1 || (obj->IsGhost() && ghost_mode_ != GhostMode::RESTART) || (!obj->IsGhost() && ghost_mode_ != GhostMode::RESTARTING))
    {
      ...
      if (!(obj->IsGhost() && GetGhostMode() == GhostMode::RESTART))  // skip ghost sample during restart
      {
        if (obj->trail_.GetNumberOfVertices() == 0 || simulationTime_ - obj->trail_.GetVertex(-1)->time > GHOST_TRAIL_SAMPLE_TIME)
        {
          ...
          {
            obj->trail_.AddVertex(
                {0.0, obj->pos_.GetX(), obj->pos_.GetY(), obj->pos_.GetZ(), obj->pos_.GetH(), simulationTime_, obj->GetSpeed(), 0.0, false});
          }
        }
      }
    }
    ...
    obj->ClearDirtyBits(Object::DirtyBit::VELOCITY | Object::DirtyBit::ANGULAR_RATE | Object::DirtyBit::ACCELERATION | Object::DirtyBit::ANGULAR_ACC);
  }
}
            ...
            for (size_t n = 0; n < event->action_.size(); n++)
            {
              ...
              if (action->base_type_ == OSCAction::BaseType::PRIVATE)
              {
                ...
                if (pa->object_ == object)
                {
                  ...
                  if (pa->type_ == OSCPrivateAction::ActionType::LONG_SPEED || pa->type_ == OSCPrivateAction::ActionType::LONG_SPEED_PROFILE ||
                      pa->type_ == OSCPrivateAction::ActionType::LAT_LANE_CHANGE || pa->type_ == OSCPrivateAction::ActionType::LAT_LANE_OFFSET ||
                      pa->type_ == OSCPrivateAction::ActionType::SYNCHRONIZE_ACTION || pa->type_ == OSCPrivateAction::ActionType::FOLLOW_TRAJECTORY ||
                      pa->type_ == OSCPrivateAction::ActionType::ASSIGN_ROUTE || pa->type_ == OSCPrivateAction::ActionType::TELEPORT)
                  {
                    // Replace object
