  ...
  else if (following_mode_ == FollowingMode::FOLLOW && vertex.size() == 2)
  {
    ...
    else
    {
      ...
      if (factor < 0.0)
      {
        ...
        {
          LOG("SpeedProfile: Can't reach target speed %.2f on target time %.2fs with given jerk constraints, extend to %.2fs", v3, vertex[1].t_, t3);
        }
      }
      else
      {
...
void SynchronizeAction::Step(double simTime, double dt)
{
  ...
  if (master_object_->pos_.GetTrajectory() || !master_object_->pos_.Delta(target_position_master_, diff))
  {
    ...
    diff.ds =
        GetLengthOfLine2D(master_object_->pos_.GetX(), master_object_->pos_.GetY(), target_position_master_->GetX(), target_position_master_->GetY());
  }
