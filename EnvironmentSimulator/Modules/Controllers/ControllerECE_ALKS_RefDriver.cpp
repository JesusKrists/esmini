  ...
  for (size_t i = 0; i < entities_->object_.size(); i++)
  {
    ...
    if (object_->pos_.Delta(&entities_->object_[i]->pos_, diff, lookaheadDist) == true)
    {
        ...
        if (!driverBraking_ && ((diff.dLaneId == -1 && targetVT > 0 && targetO > 0.375) || (diff.dLaneId == 1 && targetVT < 0 && targetO < -0.375)))
        {
          ...
          if (TTC >= 0 && targetV < egoV)
          {
            ...
            if (TTC < 2)
            {
              ...
              // after 1.15sec including braking delay) TTC (< 2sec) + offset (> 0.375) 0.75sec braking delay + 0.4sec risk perception time (distance
              // a and b in plot of regulation)
              waitTime_ = 1.15;
              ALKS_LOG(
      ...
      else if (diff.dLaneId == 0 && diff.ds > 0)  // dLaneId == 0 indicates there is linked path between object lanes, i.e. no lane changes needed
      {
        ...
        // cut-out with object on same lane, but with a distance of more than 0.375m from lane center and lateral velocity into opposite direction of
        // ego
        ...
        else if (!driverBraking_ && ((targetO > 0.375 && targetVT < 0) || (targetO < -0.375 && targetVT > 0)))
        {
          ...
          if (TTC < 2)
          {
            // cut-in would be in the red zone in front of ego vehicle after lane change (at least after 0.4sec risk perception time or after 1.15sec
            // including braking delay) TTC (< 2sec) + offset (> 0.375) 0.75sec braking delay + 0.4sec risk perception time (distance a and b in plot
            // of regulation)
            waitTime_ = 1.15;
            ALKS_LOG(
        ...
        if (targetAS < 0 && dsFree >= 0 && TTC < 2)  // TTC of object in front < 2sec
        {
          ...
          // in case of default scenarios the next is never happening because of full wrap and braking by AEB, that brakes even harder than the
          // driver.
