                                       ...
                                       EntityScaleMode         scaleMode)
{
  ...
  if (modelgroup == nullptr)
  {
    ...
    {
      LOG("Failed to load visual model %s. %s", modelFilepath.c_str(), file_name_candidates.size() > 1 ? "Also tried the following paths:" : "");
    ...
    if (boundingBox != nullptr &&
        !(boundingBox->dimensions_.length_ < static_cast<float>(SMALL_NUMBER) && boundingBox->dimensions_.width_ < static_cast<float>(SMALL_NUMBER) &&
          boundingBox->dimensions_.height_ < static_cast<float>(SMALL_NUMBER)))
    {
      geode->addDrawable(new osg::ShapeDrawable(new osg::Box(osg::Vec3(boundingBox->center_.x_, boundingBox->center_.y_, boundingBox->center_.z_),
  ...
  if (scaleMode == EntityScaleMode::NONE || scaleMode == EntityScaleMode::MODEL_TO_BB)
  {
    ...
    if (boundingBox != nullptr &&
        !(boundingBox->dimensions_.length_ < static_cast<float>(SMALL_NUMBER) && boundingBox->dimensions_.width_ < static_cast<float>(SMALL_NUMBER) &&
          boundingBox->dimensions_.height_ < static_cast<float>(SMALL_NUMBER)))
    {
      bbGeode->addDrawable(new osg::ShapeDrawable(new osg::Box(osg::Vec3(boundingBox->center_.x_, boundingBox->center_.y_, boundingBox->center_.z_),
                                                               boundingBox->dimensions_.length_,
                                                               boundingBox->dimensions_.width_,
                                                               boundingBox->dimensions_.height_)));
    }
      ...
      for (int j = 0; j < lane_section->GetNumberOfLanes(); j++)
      {
        ...
        for (int k = 0; k < 2; k++)
        {
          ...
          for (int m = 0; m < static_cast<int>(curr_osi->GetPoints().size()); m++)
          {
            ...
            vertices->push_back(
                osg::Vec3(static_cast<float>(osi_point_s.x), static_cast<float>(osi_point_s.y), static_cast<float>(osi_point_s.z + z_offset)));
          }
...
int Viewer::CreateRoadSignsAndObjects(roadmanager::OpenDrive* od)
{
  ...
  for (int r = 0; r < od->GetNumOfRoads(); r++)
  {
    ...
    for (size_t o = 0; o < static_cast<unsigned int>(road->GetNumberOfObjects()); o++)
    {
      ...
      else if (object->GetType() == roadmanager::RMObject::ObjectType::TREE || object->GetType() == roadmanager::RMObject::ObjectType::VEGETATION)
      {
      ...
      else
      {
        ...
        for (; nCopies < 1 || (rep && rep->length_ > SMALL_NUMBER && cur_s < rep->GetLength() && cur_s + rep->GetS() < road->GetLength()); nCopies++)
        {
          ...
          if (rep == nullptr)
          {
            ...
            clone->setPosition(
                osg::Vec3(static_cast<float>(pos.GetX()), static_cast<float>(pos.GetY()), static_cast<float>(object->GetZOffset() + pos.GetZ())));
...
bool ViewerEventHandler::handle(const osgGA::GUIEventAdapter& ea, osgGA::GUIActionAdapter&)
{
  ...
  switch (ea.getKey())
  {
    ...
    case (osgGA::GUIEventAdapter::KEY_I):
    {
      ...
      {
        int mask =
            viewer_->GetNodeMaskBit(viewer::NodeMask::NODE_MASK_INFO | viewer::NodeMask::NODE_MASK_INFO_PER_OBJ) / viewer::NodeMask::NODE_MASK_INFO;
