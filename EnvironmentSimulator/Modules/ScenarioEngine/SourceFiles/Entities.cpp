...
double Object::PointCollision(double x, double y)
{
  ...
  for (int j = 0; j < 2; j++)  // for longitudinal and lateral sides
  {
    ...
    // Specify bounding box corner vertices, starting at first quadrant
    double vertices[4][2] = {{static_cast<double>(obj0->boundingbox_.center_.x_) + static_cast<double>(obj0->boundingbox_.dimensions_.length_) / 2.0,
    ...
    if (((min[0] < dot_p - SMALL_NUMBER) && (max[0] < dot_p - SMALL_NUMBER)) || ((max[0] > dot_p + SMALL_NUMBER) && (min[0] > dot_p + SMALL_NUMBER)))
    {
