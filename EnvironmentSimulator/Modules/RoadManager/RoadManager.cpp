...
int Road::GetLaneInfoByS(double s, int start_lane_section_idx, int start_lane_id, LaneInfo& lane_info, int laneTypeMask)
{
  ...
  else
  {
    ...
    if (s > lane_section->GetS() + lane_section->GetLength() - SMALL_NUMBER || s < lane_section->GetS())
    {
      ...
      {
        while (s > lane_section->GetS() + lane_section->GetLength() - SMALL_NUMBER && lane_info.lane_section_idx_ + 1 < GetNumberOfLaneSections())
        {
...
void RoadLink::Print()
{
  cout << "RoadLink type: " << type_ << " id: " << element_id_ << " element type: " << element_type_ << " contact point type: " << contact_point_type_
       << endl;
}
        ...
        else if (!strcmp(child->name(), "laneSection"))
        {
          ...
          for (pugi::xml_node_iterator child2 = child->children().begin(); child2 != child->children().end(); child2++)
          {
            ...
            for (pugi::xml_node_iterator lane_node = child2->children().begin(); lane_node != child2->children().end(); lane_node++)
            {
              ...
              else if (!strcmp(lane_node->attribute("type").value(), "entry") || !strcmp(lane_node->attribute("type").value(), "mwyEntry"))
              {
                lane_type = Lane::LANE_TYPE_ENTRY;
              }
              else if (!strcmp(lane_node->attribute("type").value(), "exit") || !strcmp(lane_node->attribute("type").value(), "mwyExit"))
              {
              ...
              for (pugi::xml_node roadMark = lane_node->child("roadMark"); roadMark; roadMark = roadMark.next_sibling("roadMark"))
              {
                ...
                if (roadMark.attribute("weight") != 0 && strcmp(roadMark.attribute("weight").value(), ""))
                {
                  ...
                  {
                    LOG("unknown lane road mark weight: %s (road id=%d) setting to standard", roadMark.attribute("type").value(), r->GetId());
                    roadMark_weight = LaneRoadMark::STANDARD;
                  }
                }
                ...
                if (lane_roadMarkType == 0)
                {
                  if (roadMark_type == LaneRoadMark::NONE_TYPE)
                  {
                    ...
                    lane_roadMark->AddType(std::shared_ptr<LaneRoadMarkType>{lane_roadMarkType});
                    LaneRoadMarkTypeLine::RoadMarkTypeLineRule rule = LaneRoadMarkTypeLine::NONE;
                    LaneRoadMarkTypeLine* lane_roadMarkTypeLine     = new LaneRoadMarkTypeLine(0, 0, 0, 0, rule, roadMark_width, roadMark_color);
                    lane_roadMarkType->AddLine(std::shared_ptr<LaneRoadMarkTypeLine>{lane_roadMarkTypeLine});
                  }
                  ...
                  else if (roadMark_type == LaneRoadMark::BROKEN)
                  {
                    ...
                    lane_roadMark->AddType(std::shared_ptr<LaneRoadMarkType>{lane_roadMarkType});
                    LaneRoadMarkTypeLine::RoadMarkTypeLineRule rule = LaneRoadMarkTypeLine::NONE;
                    LaneRoadMarkTypeLine* lane_roadMarkTypeLine     = new LaneRoadMarkTypeLine(4, 8, 0, 0, rule, roadMark_width, roadMark_color);
                    lane_roadMarkType->AddLine(std::shared_ptr<LaneRoadMarkTypeLine>{lane_roadMarkTypeLine});
                  }
          ...
          for (int i = 0; i < lane_section->GetNumberOfLanes(); i++)
          {
            ...
            {
              LOG("Warning: expected laneId %d missing of roadId %d. Found laneIds %d and %d", lastLaneId - 1, r->GetId(), lastLaneId, lane->GetId());
            }
        ...
        if (connection->GetIncomingRoad()->GetId() == road1_id)
        {
          ...
          if (exit_link->GetElementId() == road2_id)
          {
            ...
            for (int j = 0; j < lane_section->GetNumberOfLanes(); j++)
            {
              ...
              if ((connection->GetContactPoint() == ContactPointType::CONTACT_POINT_START && lane_link_predecessor->GetId() == lane1_id &&
      ...
      else
      {
        ...
        if (link[i]->GetElementId() != connection->GetIncomingRoad()->GetId())
        {
          ...
          for (int j = 0; j < 2; j++)
          {
            if (link2[j] != 0)
            {
              if (link2[j]->GetElementType() == RoadLink::ElementType::ELEMENT_TYPE_JUNCTION && link2[j]->GetElementId() == junction->GetId())
              {
    ...
    for (int j = 0; j < number_of_lane_sections; j++)
    {
      ...
      for (int k = 0; k < number_of_lanes; k++)
      {
        ...
        while (++counter)
        {
          ...
          {
            if (DistanceFromPointToLine2DWithAngle(pos_candidate.GetX(), pos_candidate.GetY(), pos_pivot.GetX(), pos_pivot.GetY(), pos_pivot.GetH()) <
                min_segment_length)
            {
  ...
  else if (road_link->GetElementType() == RoadLink::ELEMENT_TYPE_JUNCTION)
  {
    ...
    else
    {
      ...
      if (GetRoute() && GetRoute()->IsValid() && GetRoute()->waypoint_idx_ >= 0)
      {
        ...
        for (int i = 0; i < n_connections; i++)
        {
          LaneRoadLaneConnection lane_road_lane_connection = junction->GetRoadConnectionByIdx(road->GetId(), lane->GetId(), i, snapToLaneTypes_);
  ...
  for (int i = 0; done == false && i < max_links; i++)
  {
    ...
    else if (ret_val2 >= ReturnCode::OK)
    {
      ...
      else if (road->GetLaneWidthByS(GetS(), li.lane_id_) < SMALL_NUMBER)
      {
        ...
        int    new_lane_idx =
            road->GetLaneSectionByIdx(li.lane_section_idx_)->GetClosestLaneIdx(GetS(), GetT(), SIGN(lane_id_), offset, true, snapToLaneTypes_);
        if (new_lane_idx < 0)
        {
          ret_val = ReturnCode::ERROR_GENERIC;
        }
...
void NurbsShape::CalculatePolyLine()
{
  ...
  for (size_t i = 0; i < ctrlPoint_.size(); i++)
  {
    ...
    if (i > 0)
    {
      length_ += PointDistance2D(ctrlPoint_[i - 1].pos_.GetX(), ctrlPoint_[i - 1].pos_.GetY(), ctrlPoint_[i].pos_.GetX(), ctrlPoint_[i].pos_.GetY());
    }
  }
  ...
  if (minimal_waypoints_.size() > 0)
  {
    ...
    if (position->GetTrackId() == minimal_waypoints_.back().GetTrackId())
    {
      ...
      else  // at least two road-unique waypoints
      {
        ...
        LOG("Removing previous waypoint for same road %d (at s %.2f)", minimal_waypoints_.back().GetTrackId(), minimal_waypoints_.back().GetS());
