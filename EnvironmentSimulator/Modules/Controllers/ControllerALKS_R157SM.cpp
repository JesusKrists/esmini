...
int ControllerALKS_R157SM::Model::Detect()
{
      ...
      (candidate_obj_info.obj != object_in_focus_.obj || candidate_obj_info.action != object_in_focus_.action))
  {
    ...
    R157_LOG(1, "Detected object: %s Scenario: %s", candidate_obj_info.obj->GetName().c_str(), ScenarioType2Str(candidate_obj_info.action).c_str());
  }
  ...
  if (veh_->pos_.Delta(&info.obj->pos_, diff, false, GetMaxRange()) == true)
  {
         ...
         (diff.dLaneId > -2 && diff.dLaneId < 2)))           // consider other entities in own or adjecent lanes
    {
      ...
      if (veh_->FreeSpaceDistanceObjectRoadLane(info.obj, &info.dist_lat, &info.dist_long, roadmanager::CoordinateSystem::CS_ROAD) == 0)
      {
        ...
        info.dv_s =
            (veh_v_s - obj_v_s) * (abs(GetAngleInIntervalMinusPIPlusPI(veh_->pos_.GetHRelative())) > M_PI_2 ? -1.0 : 1.0);  // ignore road direction
...
bool ControllerALKS_R157SM::ReferenceDriver::CheckPerceptionCutIn()
{
  if (GetPhase() == Phase::INACTIVE)
  {
    ...
    else
    {
      ...
      if (lateral_dist_trigger_ && lateral_dist_trigger_->Evaluate() == true)
      {
        R157_LOG(2, "Trig perception on lateral distance: %.2f (>%.2f)", lateral_dist_trigger_->GetDistance(), lateral_dist_trigger_->threshold_);
        trig = true;
      }
